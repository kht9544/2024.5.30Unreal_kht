    ...
    if (butncheck == true)
    {
        ...
        if (Player)
        {
            if (_Items.IsValidIndex(SelectedItemIndex))
            {
                ...
                if (NewItem)
                {
                    if (Player->GetInvenCom()->GetMoney() >= _Items[SelectedItemIndex].Price)
                    {
                        Player->GetInvenCom()->AddMoney(-_Items[SelectedItemIndex].Price); 
                        ...
                        Player->GetInvenCom()->AddItem(NewItem);
                        Player->GetStatCom()->AddAttackDamage(_Items[SelectedItemIndex].Atk);
                        NewItem->Disable();
                    }
                }
